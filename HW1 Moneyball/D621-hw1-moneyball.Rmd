...
+ At 0.3635, the adjusted $R^2$ is not as high as we would prefer, and it indicates that only 36% of the variance in the response variable can be explained by the predictor variables.
...
+ At 0.2275, the adjusted $R^2$ indicates that this model explains less of variance in the response variable than Model #1.
...
+ Then we will remove one variable at a time by greatest p-value until we have a parsimonious model with only statistically significant variables. 

+ Along the way, we will take note if the removal of a variable causes a significant change in adjusted R-squared.

+ TEAM_PITCHING_SO was removed because it didn't have the expected sign. 
