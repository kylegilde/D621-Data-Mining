...
+ The side-by-side density plots in the 1st column reveal that `INCOME` appears to be higher for customers who have had an accident. In our models, let's see if this variable defies conventional wisdom is positively correlated with accidents.
...
We will use the distribution differences seen in the side-by-side box plots to create the following new dummy variables. In our modeling, if they end up being statistically significant, we may be able to create more parsimonious models with fewer dummy variables created from the many-value categorical variables.
...
```{r BCN}
transf_v1_lm <- lm(TARGET_AMT~.-TARGET_FLAG, data = train_transformed)

PT <- car::powerTransform(transf_v1_lm, family = "bcnPower")
summary(PT)
#, data = train_transformed)
train_transformed$TARGET_AMT <- car::bcnPower(imputed_train$ximp$TARGET_AMT, lambda = PT$lambda, gamma = PT$gamma)

# load_install("BIGL")
# 
# t <- ((imputed_train$ximp$TARGET_AMT + PT$gamma)^PT$lambda - 1) / PT$lambda
# summary(t)
# 
# summary((imputed_train$ximp$TARGET_AMT + PT$gamma)^PT$lambda)
# 
hist(train_transformed$TARGET_AMT)
qqnorm(train_transformed$TARGET_AMT)
summary(train_transformed$TARGET_AMT)
# 
# hist((imputed_train$ximp$TARGET_AMT +.01)^.3333)
# qqnorm((imputed_train$ximp$TARGET_AMT +.01)^.3333)
# 
# .001^-2
# load_install("rcompanion")
# rcompanion::transformTukey(imputed_train$ximp$TARGET_AMT[1:5000])
```
...
Our second model used the same BIC selection processed but used the transformed variable set with five new variables. The `TARGET_AMT` was transformed using the negative Box-Cox function, which means we will need to inverse the model's coefficients to make them meaningful. At sixteen variables, this model has five more statistically significant variables than the previous model, including all five of the transformed dummy variables we created. Moreover, adjusted R-squared is much higher at .21. This model explains much more of the variance in the response variable `TARGET_AMT`.
...
After taking the inverse of the negative Box-Cox transformation, 

```{r}
#removed_variables(BIC_orig_var_lm, BIC_transf_var_lm)
...
inversed_coef_df <- 
  data.frame(var = names(inversed_coef_value)) %>% 
  mutate(
    transformed_coef_value = coef(BIC_transf_var_lm),
    inversed_coef_value = BCN_inverse(transformed_coef_value, PT$lambda, PT$gamma)
  ) %>% 
  arrange(-transformed_coef_value)

kable(inversed_coef_df, digits = 0, format.args = list(big.mark = ',', scientific = F, drop0trailing = T))
