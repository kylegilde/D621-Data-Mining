...
Our second model applied the same BIC selection processed to the transformed variable set with five new variables. This version contains one more variable than the previous model, and it found three of the new variables to be statistically significant. However, adjusted R-squared is only slightly higher than the first model and still remains tiny.
...
Given nonnormality and kurtosis we saw earlier, let's perform a negative Box-Cox transformation on TARGET_AMT. The negative Box-Cox transformation can accommodate all of the zero values that the variable has.
...
In the diagnostic plots for model #3, the Box-Cox transformation did improve the normality of our residuals. However, it made the nonconstant variance much worse.
...
Even though we still need to figure out how to inverse the negative Box-Cox transformation and interpret the model coefficients, the third model performed the best of the three. Not only is the adjusted R-squared much higher, but the predicted R-squared is nearly as high, which indicates that we have not overfit the model.
