...
# required packages
packages <- c("prettydoc","tidyverse", "caret", "pROC", "DT", "knitr", "ggthemes", "Hmisc", "psych", "corrplot", "reshape2", "car", "MASS", "ResourceSelection", "boot", "tinytex", "VIM", "GGally", "missForest", "DMwR", "nortest", "DescTools") 
...
#Corrrelations with Transformations with TARGET
...
# 
# some_vars <- cbind(squared_vars, sqrt_vars, log_vars, train_data)
# 
# #interaction variables
# iteraction_vars <- data.frame(t(apply(some_vars, 1, combn, 2, prod)))
# colnames(iteraction_vars) <- combn(names(some_vars), 2, paste, collapse=".")
# 
# all_vars <- 
#   iteraction_vars %>% 
#   mutate(
#     TARGET_sqrt = sqrt(train_data$TARGET),
#     TARGET2 = train_data$TARGET^2,
#     TARGET_log = log(train_data$TARGET + .01)
#     )
# str(all_vars)
# 
# # reorder columns
# all_vars <- 
#   cbind(all_vars[, str_detect(names(all_vars), "TARGET")],
#         all_vars[, !str_detect(names(all_vars), "TARGET")])
# 
# str(all_vars)
...
#   correlation_df() %>% 
#   subset((Var2 %like% "%TARGET%" | Var1  %like%  "%TARGET%") 
#          & !(Var2 %like% "%TARGET%" & Var1  %like%  "%TARGET%"))

#squared variables
squared_vars <-
  apply(train_data, 2, function(x) x^2) %>%
  as.data.frame()
colnames(squared_vars) <- paste0(names(squared_vars), "2")
...
#log variables
log_vars <-
  apply(train_data, 2, function(x) log(x + .01)) %>%
  as.data.frame()
colnames(log_vars) <- paste0(names(log_vars), "_log")
...
# combine all dfs
all_vars <-
  cbind(iteraction_vars, squared_vars, sqrt_vars, log_vars, train_data)
str(all_vars)
# reorder columns
all_vars <-
  cbind(all_vars[, str_detect(names(all_vars), "TARGET")],
...
all_corr <- all_corr[!duplicated(all_corr[, 2]),]


rownames(all_corr) <- 1:nrow(all_corr)

kable(head(all_corr, 200), digits = 2, row.names = T, caption = "Top Corrrelations with the Response Variable")
##https://stackoverflow.com/questions/35891340/row-wise-sorting-in-r?utm_medium=organic&utm_source=google_rich_qa&utm_campaign=google_rich_qa
