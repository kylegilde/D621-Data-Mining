...
+ Because of the **skewed distributions** for `age` & `lstat`, we will follow Sheather's quote of Cook & Weisberg on page 284 and add **log terms** to the model. 
...
+ Contrary to what we expected, the proporition of non-retail business `indus` has a negative coefficient. Having access to radial highways `rad` and median owner-occupied home values `medv` have actually have positive coefficients instead of the expected negative ones. This suggests that we still have some multicollinearity to deal with.
...
+ However, the model fails the Hosmer-Lemeshow goodness-of-fit test. With a p-value at .023, we have to reject the Hosmer-Lemeshow null hypothesis that the model does have a good fit.

+ With 3 variables having a variance inflation factor of greater than 4 (`VIF_gt_4`), we do see evidence of multicollinearity.

+ The area under the ROC curve (`AUC`) is relatively high at .974.
...
+ In this model, only 5 of the 17 variables have stat-sig p-values and only 1 of the transformed variables is stat-sig.
...
+ In the model's summary statistics below, this model's p-value & deviance have both decreased from the previous 12-variable model. However, Hosmer-Lemeshow goodness-of-fit p-value has declined as well, which indicates that we have to reject the Hosmer-Lemeshow null hypothesis that the model does have a good fit.

+ Since 13 of the variables have a variance inflation factor of greater than 4 (`VIF_gt_4`), the  multicollinearity issues have increased.
...
+ Our selection processed removed 8 of the variables, including 3 of the transformations, which didn't prove to be statistically significant to the model.
...
+ Addtionally, only 2 variables in the model have a variance inflation factor of greater than 4 (`VIF_gt_4`), which indicates that there are less  multicollinearity issues than the previous models.
...
```{r}
larger_mod <- update(base_model_plus, .~.^2)
test <- MASS::stepAIC(larger_mod, trace = 0)
```
...
removed_variables <- function(larger_mod, smaller_mod){
  print("removed variables")
  names(coef(larger_mod))[!names(coef(larger_mod)) %in%
...
+ Addtionally, more than half of the variables have a variance inflation factor of greater than 4, which indicates that there are more multicollinearity issues than the previous model.
...
+ Our selection processed removed 9 of the variables, including 4 of the 5 variable transformations.
...
+ Moreover, these model does not have any of the multicollinearity issues of the previous models.
...
+ With 8 variables, it is the most parsimonious.
...
However, a plot of the Standardized Deviance Values against the leverage values shows that we have several observations greater than twice the average leverage value. This indicates that we may want to consider other transformations.
