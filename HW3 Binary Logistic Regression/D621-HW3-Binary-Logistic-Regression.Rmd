...
+ If we employ the heuristic that crime-prone areas alre more likely to have less desirable characteristics, we would suspect that the crime rate might be positively correlated with having industry (`indus`), pollution (`nox`) pupil-teacher ratios (`ptratio`) & lower social status (`lstat`). 

+ Conversely, we would expect that the crime rate would be inversely related to the rate of large residential lots (`zn`), the average rooms per dwelling (`rm`), access to radial highways (`rad`) and median owner-occupied home values (`medv`)

+ Without knowing more about 1970s Boston, it's difficult to hypothesize on the relationship of the crime rate to bordering the Charles River (`chas`), having owner-occupied homedbuilt before 1940 (`age`), the distance to Boston's employment centers (`dis`) & the property tax rate per $10,000. (`tax`)
...
kable(cor_df, digits=4, row.names = T, caption = "Top Correlated Variable Pairs")

#dedupe the rows
# sort_rows <- t(apply(corr_list, 1, sort, decreasing = T))
# fin_list <- corr_list[!base::duplicated(sort_rows), ]

target_corr <- subset(cor_df, Var2 == "target" | Var1 == "target")

rownames(target_corr) <- 1:nrow(target_corr)

kable(target_corr, digits=4, row.names = T, caption = "Top Corrrelations  Variable Pairs")
```
